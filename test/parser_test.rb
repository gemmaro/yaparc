...
class YaparcTest < Test::Unit::TestCase
  ...
  def test_seq_parse
    ...
    parser = Seq.new(Item.new, Item.new, Item.new) do |item1, _item2, item3|
      [item1, item3]
    end
    result = parser.parse('ABCDEF')
    assert_equal %w[A C], result.value
    assert_equal 'DEF', result.input

    parser = Seq.new(Item.new, Item.new, Item.new) do |_item1, item2, _item3|
      [item2]
    end
    result = parser.parse('ABCDEF')
    ...
    parser = Seq.new(
      Yaparc::Apply.new(Yaparc::Tokenize.new(Digit.new)) do |digit|
        digit
      end, Many.new(Char.new('a'), ''), Many.new(Digit.new, '')
    ) do |digit, _, digits|
      digit + digits
    end
    result = parser.parse('123abc')
  ...
  def test_regex_parse
    ...
    assert_equal %w[ab 1234], result.value
  end
  ...
  def test_many_parse
    is_digit = Satisfy.new(->(i) { i >= '0' and i <= '9' })
...
class IdentifierParserTest < Test::Unit::TestCase
  ...
  def test_parse_with_keyword
    parser_with_keyword = Identifier.new(exclude: %w[abc efg])
